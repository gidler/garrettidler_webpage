...
- `/nfl-quick-pick/`: NFL Quick Pick, copied from `../nfl_elimination/index_8bit.html` to `nfl-quick-pick/index.html`. The pixel font is embedded; icons and the share-preview image live in `nfl-quick-pick/assets/`. Update by copying the HTML and its referenced assets from the source folder, checking locally, then committing and pushing.
